...
IMAGE_NAME="Ikhyeon-Cho/pytorch:$PYTORCH_VERSION-cu$CUDA_VERSION-ubuntu$UBUNTU_VERSION-$PYTHON_VERSION"
...
if [[ "$(docker ps -a -q -f name=$CONTAINER_NAME)" ]]; then
  ...
  docker run -it --name "$CONTAINER_NAME" \
    ...
    -v "$SCRIPT_DIR/conda_install.sh":"/root/conda_install.sh" \
    -w "/workspace" \
    "$IMAGE_NAME" \
    bash -c "/root/conda_install.sh cuda-${CUDA_VERSION%.*} && $SHELL"
fi